...
const Env = {
  WS_BASE_URL: 'http://10.8.90.17:8888',
  K6_DURATION: '10s',
  K6_VUS: 1
};
...
function generateSignStr(acntNo: string, subAcntNo: string, symbol: string, ordrQty: string, ordrUntprc: string, ordrTrdTp: string, buySelTp: string, oddOrdrYn: string, uuid: string, privateKey: string) {
...
test('Playwright API test - NewOrder flow', async ({ request }) => {
  ...
  if (!loginData.error) {
    ...
    const acntInfo = loginData.data.custInfo?.normal?.find((a: any) => a.subAcntNo.startsWith('N')) || {};
    ...
    const signStr = generateSignStr(acntNo, subAcntNo, symbol, ordrQty, ordrUntprc, ordrTrdTp, buySelTp, oddOrdrYn, rqId, privateKey);
